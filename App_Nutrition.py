...
with st.sidebar:
    ...
    st.divider()
    st.header("👤 Your Physical Profile")
    ...
    st.success(f"🎯 **Daily Calorie Target:** {int(target_calories)} kcal")
    st.info(f"💪 **Estimated Protein Target:** {int(weight * 1.8)}g / day")
...
- Primary Goal: {goal}
- Estimated Daily Calorie Target: {int(target_calories)} kcal (Breakfast: {b_cal} kcal, Lunch: {l_cal} kcal, Dinner: {d_cal} kcal)
- Recommended Protein: {int(weight * 1.8)}g/day
...
SYSTEM_PROMPT = f"""You are a world-class certified nutritionist, dietitian, and master chef.
{profile_info}

CRITICAL LANGUAGE RULE:
- ALWAYS respond in the following user-selected language: **{selected_language}**.
- If Urdu script is selected, use proper Urdu script (اردو).
- If Hindi script is selected, use proper Devanagari script (हिंदी).
- If Roman Urdu / Hindi is selected, use Latin alphabet script (e.g. "Aap yeh recipe try karein").
- If Japanese is selected, use proper Japanese (日本語).
- If Chinese is selected, use proper Simplified Chinese (中文).

CORE RESPONSIBILITIES:
1. **Greetings & Casual Chat:** Respond politely in {selected_language} and ask how you can assist them with diet, health, height increase, or recipes.
2. **Height Gain / Growth Guidance:**
   - Provide diet plans rich in Calcium, Vitamin D, Protein, Zinc, and Magnesium for bone growth and height increase.
   - Give practical tips on posture, sleep (HGH release), and stretch exercises alongside diet.
3. **Medical Condition Compliance (STRICT):**
   - Diabetes: Low GI, zero refined sugar.
   - High BP: Low sodium, half tastemaker packet for instant noodles.
   - High Cholesterol/Fatty Liver: Low oil, high fiber.
   - Uric Acid: Low purine.
4. **Maggi & Instant Noodles Hacks:** Suggest healthy modifications (add veggies, egg whites, half masala packet).
5. **Limited Ingredient Guides (Fruit Chaat / Pantry):** Give portion guides for 2-3 fruits or minimal pantry items per person (150-200g fruit serving).
6. **Comprehensive Recipes:**
   - ⏱️ Prep & Cook Time
   - 🛒 Ingredients & Exact Quantities
   - 👨‍🍳 Step-by-Step Cooking Instructions
   - 💡 Medical Tweaks & Health Advice
...
with f_col1:
    selected_ingredient = st.selectbox(
        ...
        ["Chicken", "Beef", "Mutton", "Fish / Seafood", "Egg", "Paneer", "Daal / Legumes", "Chickpeas", "Rice Dish", "Pasta Dish", "Maggi / Instant Noodles", "Fruit Chaat / Salads"]
    )
...
if col1.button("📏 Height Increase Diet"):
    triggered_prompt = f"Give me a complete daily diet plan and nutrition tips specifically for height increase/growth for my age ({age}) and gender ({gender}), focusing on Calcium, Vitamin D, Protein, and Zinc. Respond in {selected_language}."
elif col2.button("💪 High Protein Lunch"):
    triggered_prompt = f"Give me a high-protein lunch recipe option aimed at achieving my daily protein goal of {int(weight * 1.8)}g considering my health conditions. Respond in {selected_language}."
elif col3.button("🍜 Healthy Maggi Hack"):
    triggered_prompt = f"How can I make Maggi or Instant Noodles healthy, low sodium, and suitable for my target goal ({goal}) and medical conditions ({', '.join(health_conditions) if health_conditions else 'None'})? Respond in {selected_language}."
...
if filter_recipe_btn:
    triggered_prompt = f"Give me a delicious and healthy recipe featuring **{selected_ingredient}** in **{selected_cuisine}** style according to my goal ({goal}) and health conditions ({', '.join(health_conditions) if health_conditions else 'None'}). Respond in {selected_language}."

if pantry_submitted and pantry_items.strip():
    triggered_prompt = f"I have these ingredients in my pantry: {pantry_items}. Suggest a delicious and healthy recipe I can make with them according to my diet goal ({goal}) and health conditions! Respond in {selected_language}."

# ---------------------------------------------------------
# Chat History & Response Stream
# ---------------------------------------------------------
if "messages" not in st.session_state:
    st.session_state.messages = [
        {"role": "assistant", "content": f"Hello! I am your AI Nutritionist & Master Chef. Target: **{int(target_calories)} kcal/day**. Select your goal (including Height Increase), language in the sidebar, and ask away!"}
    ]
...
# User Chat Input
chat_input_text = st.chat_input("Ask about height increase diet, recipes, or nutrition...")
